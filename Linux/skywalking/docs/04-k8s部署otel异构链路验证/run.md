...
demo-k8s-otel-php
  -> PHP 简单 HTTP 服务
  -> 手动打印/透传 traceparent
  -> Java 使用 host.docker.internal:4317
  -> Python / Go 使用 host.docker.internal:4318
...
- OTel Collector：`http://host.docker.internal:4318`
...
PHP 服务当前不使用 Operator 自动注入，本示例先用于验证异构链路中的 HTTP 调用和上下文透传：

- `/hello`：简单接口。
- `/chain`：可选传入 `targetUrl` 请求下游服务。
- 日志：从请求头 `traceparent` 中解析并打印 `trace_id`。
- 透传：请求下游服务时继续传递当前请求头中的 `traceparent`。
...
```text
[demo-php-otel] info [trace_id=xxx] hello request, name=php
```
...
- PHP 当前不创建 OTel Span，主要通过日志中的 `trace_id` 和 `traceparent` 透传验证上下文传播
